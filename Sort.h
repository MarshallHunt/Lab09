...
template < class T > // this copies array into a new array
...
template < class T > //this takes array from above
...
template < class T > // this does swapping
int Sort<T>::partition(T** items, int first, int last, int (*compare) (T* one, T* two))
{
   ...
   choosePivot(items, first, last); 
	...
	{
		int compares = (*compare) (items[i], items[Pivot]);
		if(compares < 0)
		{
			lastS1 = lastS1 + 1;
			temp = items[i];
